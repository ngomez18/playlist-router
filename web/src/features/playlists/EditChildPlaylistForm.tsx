...
interface EditChildPlaylistFormProps {
  childPlaylist: ChildPlaylist
  onSuccess: () => void
  onCancel: () => void
}

export function EditChildPlaylistForm({ 
  childPlaylist, 
  onSuccess, 
  onCancel 
}: EditChildPlaylistFormProps) {
  const [formData, setFormData] = useState<{
    ...
    description: childPlaylist.description || '',
    is_active: childPlaylist.is_active,
    filter_rules: childPlaylist.filter_rules || {},
  })
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UpdateChildPlaylistRequest) => 
      apiClient.updateChildPlaylist(childPlaylist.id, data),
    onSuccess: () => {
      onSuccess()
    },
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updateData: UpdateChildPlaylistRequest = {
  ...
  return (
    <div className="card w-full max-w-2xl bg-base-200 shadow-xl border border-base-300">
      <div className="card-body">
        <h2 className="card-title">Edit Child Playlist</h2>
        <p className="text-sm text-base-content/70 mb-4">
          Update your filtered playlist settings and filter rules.
        </p>

        {updateMutation.error && (
          <Alert type="error" className="mb-4">
            {updateMutation.error.message || 'Failed to update child playlist'}
          </Alert>
        )}
          ...
          <div className="form-control">
            <label className="label cursor-pointer justify-start gap-2">
              <input
                ...
                disabled={updateMutation.isPending}
              />
              <span className="label-text">Active (receives songs from base playlist)</span>
            </label>
          </div>
